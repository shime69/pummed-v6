...
void c_menu::draw_spectators()
{
	...
	// window body
	{
		...
		list->AddText(
			...
			c_color(80,80,80, window_alpha).as_imcolor(),
			CXOR("spectators"));

		list->AddLine(
			window_pos + ImVec2(0, 31.f),
			window_pos + ImVec2(keybinds_size.x, 31),
			c_color(80,80,80, 12.75f * alpha).as_imcolor());
...
void c_menu::draw_bomb_indicator()
{
	...
	// window body
	{
		...
		// header
		imgui_blur::create_blur(list, window_pos, ImVec2(window_pos.x + bomb_size.x, window_pos.y + bomb_size.y), c_color(80,80,80, window_alpha).as_imcolor(), 8.f);
		...
		// border
		list->AddRect(window_pos, ImVec2(window_pos.x + bomb_size.x, window_pos.y + bomb_size.y), c_color(0,0,0, 100.f * alpha).as_imcolor(), 8.f);
...
void c_menu::draw_watermark()
{
	...
	imgui_blur::create_blur(list, window_pos, window_pos + window_size, ImColor(80,80,80, (int)(130 * alpha)), 4.f, ImDrawCornerFlags_All);
